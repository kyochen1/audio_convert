...
#pragma endregion
	...
	public:
		...
		// PCM双字节双声道数据分离（left_data，right_data需要手动清理）
		...
		// PCM单字节数据转换PCM双字节数据（dst_data需要手动清理）
		int pcm8_to_pcm16(const char* const src_data, const unsigned int src_size
			, short** const dst_data, unsigned int* const dst_size, const unsigned short format=WAV_FORMAT_PCM);
		// WAV文件PCM8转换PCM16（只支持PCM,PCMA和PCMU）
		...
		int pcm16_to_pcm8(const short* const src_data, const unsigned int src_size
			, char** const dst_data, unsigned int* const dst_size, const unsigned short format = WAV_FORMAT_PCM);
		// WAV文件PCM16转PCM8（只支持PCM,PCMA和PCMU）
		int wav_pcm16_to_pcm8(const char* const src_filename, const char* const dst_filename, const unsigned short format = WAV_FORMAT_PCM);

	private:
		short alaw_to_pcm16(unsigned char val);
		short ulaw_to_pcm16(unsigned char val);
		unsigned char pcm16_to_alaw(short val);
		unsigned char pcm16_to_ulaw(short val);
	};
